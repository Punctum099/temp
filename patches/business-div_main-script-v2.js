<script>
    function goCategory(obj,idx){
        $('.biz-category ul li a').removeClass('on');
        $(obj).addClass("on");
    ...
    function bizSwiperset() {
					...
					var slideCount = $container.find('.biz-product > .biz-product-item').length;
					if (slideCount === 0) {
						return;
					}
					...
					if ($container[0].swiper) {
						...
						$container[0].swiper = null;
					}

					var options = $.extend(true, {}, bizOption, {
						loop: slideCount > 1,
						autoplay: slideCount > 1 ? bizOption.autoplay : false,
						pagination: $.extend(true, {}, bizOption.pagination, {
							formatFractionCurrent: function () {
								return ('0' + (this.realIndex + 1)).slice(-2);
							},
							formatFractionTotal: function () {
								return ('0' + slideCount).slice(-2);
							}
						})
					});

					bizSwiper = new Swiper($container[0], options);
				}
